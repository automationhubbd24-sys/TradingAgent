...
from tradingagents.dataflows.binance import load_binance_futures_ohlcv, load_binance_spot_ohlcv
...
def _configured_market_vendor() -> str:
    config = get_config()
    tool_vendor = config.get("tool_vendors", {}).get("get_stock_data")
    vendor = tool_vendor or config.get("data_vendors", {}).get("core_stock_apis", "yfinance")
    return str(vendor).split(",", 1)[0].strip().lower()


def _load_verified_ohlcv(symbol: str, curr_date: str) -> pd.DataFrame:
    vendor = _configured_market_vendor()
    if vendor == "binance_spot":
        return load_binance_spot_ohlcv(symbol, curr_date)
    if vendor == "binance_futures":
        return load_binance_futures_ohlcv(symbol, curr_date)
