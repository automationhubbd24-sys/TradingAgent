...
type Decision = { symbol?: string; direction?: string; confidence?: number; entry?: number; stop_loss?: number; take_profits?: number[]; expected_rr?: number; reason?: string; entry_status?: string; data_diagnostic?: DataDiagnostics };
...
type Message = { id: string; role: 'user' | 'assistant'; content: string; image?: string; decision?: Decision; streaming?: boolean; analysisStage?: string; diagnostics?: DataDiagnostics };
  ...
  function consumeEvent(block: string, assistantId: string) {
    ...
    try {
      ...
      setMessages(items => items.map(item => {
        ...
        if (type === 'analysis_result' && data.decision) return { ...item, decision: data.decision, diagnostics: { ...item.diagnostics, ...data.decision.data_diagnostic, reason: data.decision.reason || item.diagnostics?.reason } };
...
function AnalysisCard({ decision }: { decision: Decision }) { const unavailable = decision.direction === 'DATA_UNAVAILABLE'; return <section className={`analysis-card ${unavailable ? 'unavailable' : ''}`}><div className="card-top"><div><span>{unavailable ? 'MARKET DATA STATUS' : 'DETERMINISTIC PAPER DECISION'}</span><h2>{unavailable ? 'MARKET DATA UNAVAILABLE' : decision.symbol || 'MARKET'} {!unavailable && <small>{decision.entry_status || 'WAIT'}</small>}</h2></div>{!unavailable && <div className="confidence"><b>{Math.round((decision.confidence || 0) * 100)}%</b><span>CONFIDENCE</span></div>}</div><div className="direction"><span>{unavailable ? 'Analysis status' : 'Directional bias'}</span><strong>{unavailable ? 'Market data unavailable' : decision.direction || 'NO_TRADE'}</strong><small>{decision.reason}</small></div>{!unavailable && decision.entry !== undefined && <div className="levels"><div><span>ENTRY</span><b>{decision.entry}</b></div><div><span>STOP LOSS</span><b className="danger">{decision.stop_loss}</b></div><div><span>TP1 / RR</span><b className="profit">{decision.take_profits?.[0]} / {decision.expected_rr}</b></div></div>}</section>; }
