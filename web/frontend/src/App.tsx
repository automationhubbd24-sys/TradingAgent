...
type Decision = { symbol?: string; direction?: string; directional_bias?: string; decision_state?: string; confidence?: number; entry?: number; stop_loss?: number; take_profits?: number[]; expected_rr?: number; reason?: string; entry_status?: string; setup_zone?: { low: number; high: number; source: string }; data_diagnostic?: DataDiagnostics };
...
function AnalysisCard({ decision }: { decision: Decision }) { const unavailable = decision.decision_state === 'DATA_UNAVAILABLE' || decision.direction === 'DATA_UNAVAILABLE'; const waiting = !unavailable && decision.entry_status?.startsWith('WAIT_'); return <section className={`analysis-card ${unavailable ? 'unavailable' : ''}`}><div className="card-top"><div><span>{unavailable ? 'MARKET DATA STATUS' : 'DETERMINISTIC PAPER DECISION'}</span><h2>{unavailable ? 'MARKET DATA UNAVAILABLE' : decision.symbol || 'MARKET'} {!unavailable && <small>{decision.entry_status || 'NO_TRADE'}</small>}</h2></div>{!unavailable && <div className="confidence"><b>{Math.round((decision.confidence || 0) * 100)}%</b><span>CONFIDENCE</span></div>}</div><div className="direction"><span>{unavailable ? 'Analysis status' : 'Directional bias'}</span><strong>{unavailable ? 'Market data unavailable' : decision.directional_bias || decision.direction || 'NEUTRAL'}</strong><small>{decision.reason}</small></div>{waiting && decision.setup_zone && <div className="levels"><div><span>SETUP ZONE</span><b>{decision.setup_zone.low} – {decision.setup_zone.high}</b></div><div><span>READINESS</span><b>{decision.entry_status}</b></div></div>}{!unavailable && decision.entry_status === 'CONFIRMED' && decision.entry !== undefined && <div className="levels"><div><span>ENTRY</span><b>{decision.entry}</b></div><div><span>STOP LOSS</span><b className="danger">{decision.stop_loss}</b></div><div><span>2R / 3R</span><b className="profit">{decision.take_profits?.[0]} / {decision.take_profits?.[1]}</b></div></div>}</section>; }
